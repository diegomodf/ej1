            ...
            <h3 class="box-title">Particular</h3>  
              ...
              <table id="tabela" class="table table-bordered table-striped">
                ...
                <?php 
                        ...
                        $sql = " SELECT cpf FROM paciente WHERE idpaciente = $id";
                        ...
                        echo $novo_cpf;
                        $sql = " SELECT * FROM consultas WHERE cpf_paciente = $novo_cpf";
                        $resultado_ids = mysqli_query($link, $sql);
                         if($resultado_ids){
                              while($registros = mysqli_fetch_array($resultado_ids, MYSQLI_ASSOC)){
                                    $id = $registros['idconsultas'];
                                    $especialidade = $registros['especialidade_terapeuta'];
                                    $terapeuta = $registros['nome_terapeuta'];
                                    $consulta = $registros['status_consulta']; //SE ESTÁ PAGO OU NÃO
                                    $pagamento = $registros['status_pagamento']; //Valor que foi pago
                                    //CRIAR VARIÁVEL VALOR PARA PEGAR O VALOR DA CONSULTA;
                                    echo '<tr>';
                                    echo '<td>TESTE</td>';
                                    echo '<td>TESTE</td>';
                                    echo '<td>TESTE</td>';
